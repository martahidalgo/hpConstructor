...
#------------------------
for(spe in species){
    xref_folder <- paste0("RDatas_constructor/xref/", spe, "/")
...
# ANNOTATIONS
#------------------------
load("RDatas_constructor/annotations/annotations/go_bp_frame.RData")
save(go_bp_frame, file = paste0(edpath, "go_bp_frame.rda"), compress = "bzip2")
load("RDatas_constructor/annotations/annotations/go_bp_net.RData")
save(go_bp_net, file = paste0(edpath, "go_bp_net.rda"), compress = "bzip2")

# GO_bp_annots
for(spe in species){
    gba_folder <- paste0("RDatas_constructor/annotations/annotations/", spe, "/")
...
# Entrez_HGNC
for(spe in species){
    eh_folder <- paste0("RDatas_constructor/annotations/annotations/", spe, "/")
...
for(spe in species){
    path <- paste0("RDatas_constructor/pathways/", spe, "/temp/meta_graph_info_",
                   spe, ".RData")
    mgi <- load(path)
...
# # Separatedly
# # HSA
# mgi <- load("RDatas_constructor/pathways/hsa/temp/meta_graph_info_hsa.RData")
...
# 
# # RNO
# mgi <- load("RDatas_constructor/pathways/rno/temp/meta_graph_info_rno.RData")
...
# 
# # MMU
# mgi <- load("RDatas_constructor/pathways/mmu/temp/meta_graph_info_mmu.RData")
...
for(db in dbs){
    for(spe in species){
        load(paste0("RDatas_constructor/annofuns/annofuns_", db, "_", spe, ".RData"))
...
for(spe in species){
    for(feat in feats){
        file <- paste0("RDatas_constructor/pathways/pseudo/pmgi_", spe, "_", feat,".RData")
        pseudo <- load(file)
        pmgi <- get(pseudo)
...
# # HSA
# pmgi <- load("RDatas_constructor/pathways/pseudo/pmgi_hsa_genes.RData")
# pmgi_hsa_genes <- get(pmgi)
# save(pmgi_hsa_genes, pkg = ".")
# rm(pmgi_hsa_genes, pmgi)
# 
# pmgi <- load("RDatas_constructor/pathways/pseudo/pmgi_hsa_uniprot.RData")
# pmgi_hsa_uniprot <- get(pmgi)
# save(pmgi_hsa_uniprot, pkg = ".")
# rm(pmgi_hsa_uniprot, pmgi)
# 
# pmgi <- load("RDatas_constructor/pathways/pseudo/pmgi_hsa_GO.RData")
# pmgi_hsa_GO <- get(pmgi)
# save(pmgi_hsa_GO, pkg = ".")
# rm(pmgi_hsa_GO, pmgi)
# 
# # RNO
# pmgi <- load("RDatas_constructor/pathways/pseudo/pmgi_rno_genes.RData")
# pmgi_rno_genes <- get(pmgi)
# save(pmgi_rno_genes, pkg = ".")
# rm(pmgi_rno_genes, pmgi)
# 
# pmgi <- load("RDatas_constructor/pathways/pseudo/pmgi_rno_uniprot.RData")
# pmgi_rno_uniprot <- get(pmgi)
# save(pmgi_rno_uniprot, pkg = ".")
# rm(pmgi_rno_uniprot, pmgi)
# 
# pmgi <- load("RDatas_constructor/pathways/pseudo/pmgi_rno_GO.RData")
# pmgi_rno_GO <- get(pmgi)
# save(pmgi_rno_GO, pkg = ".")
# rm(pmgi_rno_GO, pmgi)
# 
# # MMU
# pmgi <- load("RDatas_constructor/pathways/pseudo/pmgi_mmu_genes.RData")
# pmgi_mmu_genes <- get(pmgi)
# save(pmgi_mmu_genes, pkg = ".")
# rm(pmgi_mmu_genes, pmgi)
# 
# pmgi <- load("RDatas_constructor/pathways/pseudo/pmgi_mmu_uniprot.RData")
# pmgi_mmu_uniprot <- get(pmgi)
# save(pmgi_mmu_uniprot, pkg = ".")
# rm(pmgi_mmu_uniprot, pmgi)
# 
# pmgi <- load("RDatas_constructor/pathways/pseudo/pmgi_mmu_GO.RData")
